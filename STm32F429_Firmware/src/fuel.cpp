...
float fuel::get_fuel(char char_buffer[]){
	...
	if(should_blink){
		if(last_time+2*blink_freq<Millis.get()){ // gib die Zahl aus wie z.B. "119" und schreib in den Buffer "119km"
			last_time=Millis.get();
			sprintf(local_buffer,"%3ikm",((int)fuel_diff)%1000);	// gib die zahl zurück, alles easy
		} else if(last_time+blink_freq<Millis.get()){ // wir sind mit dem Tank weiter gefahren als die Blinkgranze und wollen gerade mal nix anzeigen um das blinken zu symbolisieren
