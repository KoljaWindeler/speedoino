 ...
 ************************* timer *********************/
timer::timer(){
	every_second_timer=Millis.get()+50;   // Zeitstempel vom letzten ausführen der "every_sec" funktion
	every_qsecond_timer=Millis.get()+125;   // Zeitstempel vom letzten ausführen der "every_qsec" funktion
	every_custom_timer=Millis.get();   // Zeitstempel vom letzten ausführen der "every_qsec" funktion
}
...
void ram_info() {
	...
	free(buf);
	Serial.puts(USART1,size);
}

void timer::every_sec() {
	if((Millis.get()-every_second_timer)>=998){
#ifdef TACHO_SMALLDEBUG
		Serial.puts(USART1,("-t1"));
#endif
		every_second_timer=Millis.get();
...
//		if(pSD->sd_failed && (Millis.get()/1000)%30==0 && pSensors->m_reset->reboots_caused_by_sd_problems<2){ // just two ties .. after that: die SD!
...
//				Serial.puts_ln(USART1,("SD reboot successful, rebooting speedo!"));
//				init_speedo();
//			}
//		}
//
//		// check if MIL should be active
//		if((Millis.get()/1000)%30==0 && pSensors->CAN_active && pSensors->m_CAN->get_active_can_type()==CAN_TYPE_OBD2){
//			pSensors->m_CAN->request(CAN_CURRENT_INFO,CAN_MIL_STATUS);
//		}
//
//		// check gps
//		if((Millis.get()/1000)%10==0 && pSensors->m_gps->wait_on_gps()){
//			pSensors->m_gps->reconfigure(); // default 9600
//			pSensors->m_gps->update_rate_1Hz();
//		}
//
//		if((Millis.get()/1000)==60){
//			uint8_t result=0xff;
//			bool comm_check;
...
void timer::every_qsec() {
	if((Millis.get()-every_qsecond_timer)>=250){
#ifdef TACHO_SMALLDEBUG
		Serial.puts(USART1,("-t2"));
#endif
		every_qsecond_timer=Millis.get();
...
//			// warum so häufig? -> faden der LEDS
...
void timer::every_custom() {
//	if((signed(Millis.get()-every_custom_timer)>=pSpeedo->refresh_cycle) && (pSpeedo->refresh_cycle>1)){
